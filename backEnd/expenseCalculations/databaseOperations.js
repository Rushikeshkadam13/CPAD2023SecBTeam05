...
this.addExpenseToGroup = async function (gid, eid, uid, amount) {
  try {
    ...
    for (const paymentNode of payments) {
      ...
      existingGroup.paymentGraph.push({
        from: paymentNode.from,
        to: paymentNode.to,
        balance: paymentNode.balance,
      });
    }
...
function makePaymentGraph(gid, group) {
  ...
  while (lenders.length > 0) {
    ...
    paymentGraph.push(
      new PaymentNode(sender.uid, receiver.uid, amountTransferred)
    );
    // console.log(paymentGraph)
    sender.balance -= amountTransferred;
    receiver.balance -= amountTransferred;

    if (sender.balance !== 0) insertUserNode(sender, borrowers);

    if (receiver.balance !== 0) insertUserNode(receiver, lenders);
  }
...
this.settleTransaction = async function (transaction) {
  ...
  const receiver = userBalances.filter(
    (user) => user.uid === transaction.receiver
  );
